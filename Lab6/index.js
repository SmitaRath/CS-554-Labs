...
binnedImages: async () => {
  ...
  const members=await client.zrevrangeAsync("binnedImages",0,-1,) //async function(err,members){
    if(members.length!=0) {
  ...
  async unsplashImages(_,args){
    ...
    try{
      const  { data }  = await Axios.get("https://api.unsplash.com/photos?page="+args.pageNum+"&client_id=AyyDIuWYMVLlAjcbLdWmi_FHiO-PkIQtNcEEPwR5eLk");
      ...
      for(let arr of data){
        ...
        imageData.description=arr.description?arr.description:arr.alt_description;
        imageData.userPosted=false;
        existBin = await client.getAsync(arr.id);
        if(existBin)
        imageData.binned=true;
        else
...
async getTopTenBinnedPosts(){
  ...
  const members=await client.zrevrangeAsync("binnedImages",0,9) //async function(err,members){
    if(members.length!=0) {
      for(let err of members){
      const jsonImageFromRedis = await client.getAsync(err);
      const recomposedImage = JSON.parse(jsonImageFromRedis);
...
Mutation: {
  ...
  async deleteImage(_,args){
    ...
    if(recomposedImage.binned){
     /* client.lrem("binnedImages", 0, args.id, function(err, data){
        if(err) {
          return null;
      }
